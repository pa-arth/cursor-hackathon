/** Same observation shape as jev-ultrafast/jev_ultrafast/snapshot.js (callable). */
...
/** Training-sized observation: drop bulky runtime-only fields. */
function slimObservation(page) {
  if (!page) return null;
  return {
    url: page.url,
    title: page.title,
    ...
    text: page.text,
    scroll: page.scroll,
    actions: page.actions.map(({ rect, ...a }) => a),
...
function nodeIdFor(el) {
  ...
  if (!cache || !el) return null;
  let cur = el.nodeType === 1 ? el : el.parentElement;
  while (cur && cur !== document.documentElement) {
    if (cache.ids.has(cur)) return cache.ids.get(cur);
    cur = cur.parentElement;
  }
  return null;
}

function pickAction(observation, nodeId, preferredKind, selectValue) {
  if (!observation || nodeId == null) return null;
  const matches = observation.actions.filter((a) => a.node === nodeId);
  if (!matches.length) return null;
  if (preferredKind === "select" && selectValue != null) {
    const hit = matches.find((a) => a.kind === "select" && a.value === selectValue);
    if (hit) return hit;
  }
  if (preferredKind) {
    const hit = matches.find((a) => a.kind === preferredKind);
    if (hit) return hit;
  }
  return matches.find((a) => a.kind === "click") || matches[0];
}
