...
This lives under `slitherio/` so other hackathon projects can sit beside it at the repo root.

`jev-ultrafast/` and `kev/` here are normal folders (not git submodules).
...
- Repo-root `.gitignore` keeps `.env` / `.venv` out; no nested `.git` folders
